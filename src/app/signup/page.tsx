...
function SignupForm() {
    ...
    try {
      ...
      if (data.user) {
        toast({
          ...
          description: "Welcome to Scalerbox! Please check your email to verify your account.",
        });
        // Note: Supabase might require email verification.
        // If not, we can redirect.
        // If session is established immediately (depends on config), we can redirect.
        if (data.session) {
  ...
  const handleOAuthSignUp = async (providerName: 'google' | 'github') => {
    ...
    try {
      ...
      console.error(`${providerName} sign-up error:`, error.message);
      toast({
        title: 'Sign-up failed',
        description: error.message || `An error occurred during ${providerName} sign-up.`,
        variant: 'destructive',
      });
